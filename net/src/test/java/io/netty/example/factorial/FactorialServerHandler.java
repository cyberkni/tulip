...
public class FactorialServerHandler extends SimpleChannelInboundHandler<BigInteger> {
    ...
    public static String dateToMillis(Date date) {
        return zeroPadString(Long.toString(date.getTime()), zeroArray.length);
    }
    
    public static void main(String[] args) {
    	Formatter fmt = new Formatter();
    	System.out.println(fmt.format("%,d", 1222));
    	String dateToMillis = dateToMillis(new Date());
		System.out.println(dateToMillis);
		System.out.println(new Date(Long.parseLong(dateToMillis)));
	}
